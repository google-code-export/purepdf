package org.purepdf.pdf.fonts
{
	...
	public class TrueTypeFont extends BaseFont
	{
		...
		public function init( $ttFile: String, $enc: String, $emb: Boolean, $ttfAfm: Vector.<int>, $justNames: Boolean, $forceRead: Boolean ): void
		{
			...
			if ( StringUtils.endsWith( fileName.toLowerCase(), ".ttf" ) || StringUtils.endsWith( fileName.toLowerCase(), ".otf" ) || StringUtils
				.endsWith( fileName.toLowerCase(), ".ttc" ) )
			{
		...
		private function readBbox(): void
		{
			...
			for ( var glyph: int = 0; glyph < locaTable.length - 1; ++glyph )
			{
				...
				if ( start != locaTable[ glyph + 1 ] )
				{
					...
					bboxes[ glyph ] = Vector.<int>( [ ( rf.readShort() * 1000 ) / head.unitsPerEm, ( rf.readShort() * 1000 ) / head.unitsPerEm, ( rf
						.readShort() * 1000 ) / head.unitsPerEm, ( rf.readShort() * 1000 ) / head.unitsPerEm ] );
				}
			}
		}
